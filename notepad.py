...
class Window:
	...
	def __init__(self, root):
		...
		self.root.title("Shourya's Notepad")
		
		
		# Create textspace
		self.textspace = Text(self.root)
		self.textspace.grid(row=0, column = 2)
		
		# Create open and save buttons
		Button(self.root, text = "Save", command = self.savefile).grid(row=0, column = 1)
		Button(self.root, text = "Open", command = self.openfile).grid(row=0, column = 0)
	...
	def savefile(self):
		...
		file_name = Entry(savegui, width = 60)
		file_name.grid(row=0, column=1)
		
		Button(savegui, text = "Save as text file", command = writefile).grid(row=0, column = 2)
	...
	def openfile(self):
		...
		file_name = Entry(opengui, width = 60)
		file_name.grid(row=0, column=1)
